...
app.post("/mix", async (req, res) => {
  ...
  try {
    ...
    const command = `ffmpeg -i "${meditacionPath}" -i "${fondoPath}" -filter_complex "[0:a]highpass=f=100,asetrate=44100*0.96,atempo=1.042,aecho=0.8:0.88:60:0.2[a0];[1:a]afade=t=out:st=${fadeStart}:d=4,volume=0.2[a1];[a0][a1]amix=inputs=2:duration=first" -y "${outputPath}"`;
