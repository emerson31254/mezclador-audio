...
app.post("/mix", async (req, res) => {
  ...
  try {
    ...
    const command = `ffmpeg -i "${meditacionPath}" -i "${fondoPath}" -filter_complex "[1:a]afade=t=out:st=${fadeStart}:d=4,volume=0.4[a1];[0:a][a1]amix=inputs=2:duration=first[aout];[aout]volume=2.5" -y "${outputPath}"`;
